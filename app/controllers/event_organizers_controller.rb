class EventOrganizersController < ApplicationController
  ...
  def index
    ...
    @event_organizers = @q.result(distinct: true)
  end
  ...
  private
    ...
    def event_organizer_params
      params.require(:event_organizer).permit(:name, :logo_image, :email, :contact, :address, :city, :state, :country, :latitude, :longitude)
    end
end
